...
#             python TSS_random.py --target "$target" --budget "$budget" --accent "$accent" --feature_type "$feature" >> '39-logs.txt' 2>&1
#             echo "_____random completed______"
            ...
            do
                echo "$fxn"
                python TSS.py --target "$target" --budget "$budget" --similarity "$similarity" --eta "$eta" --accent "$accent" --fxn "$fxn" --feature_type "$feature" >> '39-logs.txt' 2>&1
            done
