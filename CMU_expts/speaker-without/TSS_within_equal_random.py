...
def get_index(list_samples, indices):
    ...
    while total_duration + list_samples[indices[index]]['duration'] <= budget(budget_size)/3:
...
#    dirs = ['kannada_male_english', 'malayalam_male_english', 'rajasthani_male_english', 'hindi_male_english', 'tamil_male_english', 'gujarati_female_english', 'manipuri_female_english', 'assamese_female_english']
    ...
    output_dir = os.path.join(base_dir, query_dir, f'TSS_output/within/budget_{budget_size}/target_{target_size}/equal_random')
    ...
    for i in [1, 2, 3]:
        ...
        for i, _dir in enumerate(dirs):
            if _dir != speaker and accent_map[_dir] == accent:
                temp_dirs.append(_dir)
                selection_file_path = base_dir + '../speaker-without/' + _dir + '/manifests/selection.json'
...
preprocess(base_dir, target_size, budget_size, speaker, feature_type)
