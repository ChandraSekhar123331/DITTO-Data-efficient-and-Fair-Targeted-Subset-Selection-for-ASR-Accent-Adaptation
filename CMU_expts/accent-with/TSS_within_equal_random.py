...
def _color_map(dirs):
    color = ["yellowgreen", "darkorange", "yellow", "royalblue", 'black', 'red', '#D3D3D3']
    #color = ["yellowgreen", "darkorange", "lightcoral", "yellow", "cyan", "royalblue", 'fuchsia', 'lawngreen', 'black', 'red', 'maroon']
    color_map = {}
...
def _plot_PCA(X, y, _dirs, pca_result, extra_result, label_map, _ax, color_map):
    ...
    g = sns.scatterplot(
        x="pca-one", y="pca-two",
        hue="label",
        palette=sns.color_palette('hls', len(_dirs)), 
        data=df,
        legend="full",
        alpha=0.6,
        ax = _ax
...
def get_index(list_samples, indices, budget_size):
    total_duration, index = 0, 0
    while total_duration + list_samples[indices[index]]['duration'] <= budget_map[budget_size]/4:
...
#    dirs = ['kannada_male_english', 'malayalam_male_english', 'rajasthani_male_english', 'hindi_male_english', 'tamil_male_english', 'gujarati_female_english', 'manipuri_female_english', 'assamese_female_english']
    ...
    output_dir = os.path.join(base_dir, query_dir, f'TSS_output_NEW/within/budget_{budget_size}/target_{target_size}/equal_random')
    ...
    for i in [1, 2, 3]:
        ...
        for i, _dir in enumerate(dirs):
            if accent_map[_dir] == accent:
                ...
                index = get_index(selection_file_list, indices, budget_size)
                selected_indices.extend([idx+len(ground_list) for idx in indices[:index]])
...
#        list_total_selection.append(Counter([accent_map[item['audio_filepath'].split('/')[-3]] for item in ground_list]))
        list_total_selection.append(Counter([item['audio_filepath'].split('/')[-3] for item in ground_list]))
        list_total_count.append(len(selected_indices))
        list_total_duration.append(sum([ground_list[idx]['duration'] for idx in selected_indices]))
...
preprocess(base_dir, target_size, budget_size, accent, feature_type)
