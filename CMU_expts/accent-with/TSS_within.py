...
def compute_subset(dirs, base_dir, ground_list, ground_features, ground_features_Y, test_features, greedyList, budget_size, target_size, accent, fxn, similarity, feature_type):
    list_total_selection, list_total_count, list_total_duration = [], [], []
    list_accent_sample_count, list_accent_sample_duration = [], []
    output_dir = os.path.join(base_dir, query_dir, f'TSS_output/within/budget_{budget_size}/target_{target_size}/{fxn}/{similarity}/{feature_type}')
...
def generate_greedyList(ground_kernel, fxn, budget_size):
    print(f'\ncreating {fxn} object\n')
    if fxn == 'FL':
        obj1 = FacilityLocationFunction(n=len(ground_kernel), mode="dense", sijs=ground_kernel, metric="euclidean", separate_rep=False)
    elif fxn == 'GC':
        obj1 = GraphCutFunction(n=len(ground_kernel), mode="dense", lambdaVal=1, ggsijs=ground_kernel, metric="euclidean", separate_rep=False)
    elif fxn == 'LogD':
        obj1 = LogDeterminantFunction(n=len(ground_kernel), mode="dense", lambdaVal=1, sijs=ground_kernel, metric="euclidean")
    else:
...
def preprocess(base_dir, target_size, budget_size, accent, similarity, feature_type):
    dirs = [f.name for f in os.scandir('../speaker_with/') if f.is_dir()]
    ...
    for i, _dir in enumerate(dirs):
        if accent_map[_dir] == accent:
            temp_dirs.append(_dir)
            selection_file_path = base_dir + '../speaker_with/' +  _dir + '/manifests/selection.json'
            ...
            ground_list_Y.extend([count]*len(selection_file_list))   
            count += 1
    ...
    return temp_dirs, query_dir, ground_list, ground_features, ground_features_Y, ground_kernel, test_features 
...
dirs, query_dir, ground_list, ground_features, ground_features_Y, ground_kernel, test_features = preprocess(base_dir, target_size, budget_size, accent, similarity, feature_type)
greedyList = generate_greedyList(ground_kernel, fxn, budget_size)
compute_subset(dirs, base_dir, ground_list, ground_features, ground_features_Y, test_features, greedyList, budget_size, target_size, accent, fxn, similarity, feature_type)
