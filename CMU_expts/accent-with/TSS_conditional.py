...
def _color_map(dirs):
    color = ["yellowgreen", "darkorange", "yellow", "cyan", "royalblue", 'lawngreen', 'black', 'red', '#D3D3D3']
    ...
    color_map = {}
    for _dir, color in zip(dirs+['query_set', 'selected_set', 'private_set', 'test_set'], color):
...
def plot_TSNE(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn):
    n = len(dirs)
...
#    _ax = fig.add_subplot(3,2,1)
    ...
    X = np.concatenate([ground_features, query_features, selected_features, private_features], axis=0)
    y = np.concatenate([ground_features_Y, np.zeros((len(query_features), 1))+n, np.zeros((len(selected_features), 1))+n+1, np.zeros((len(private_features), 1))+n+2], axis=0)
    _plot_TSNE(X, y, dirs+['query_set', 'selected_set', 'private_set'], _ax, color_map, [',']*len(dirs)+['.', '*', '+'])
...
def plot_PCA(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn):
    n = len(dirs)
    selected_features = ground_features[selected_indices]
    color_map = _color_map(dirs)
    fig = plt.figure(figsize=(30,30))
...
def plots(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn):
     plot_TSNE(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn)
     plot_PCA(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn)
...
def compute_subset(dirs, base_dir, query_dir, ground_list, ground_features, ground_features_Y, query_list, query_features, private_list, private_features, test_features, greedyList, budget_size, target_size, accent, other_accent, fxn, similarity, etaValue, feature_type):
    list_total_selection, list_total_count, list_total_duration = [], [], []
    list_accent_sample_count, list_accent_sample_duration = [], []
    output_dir = os.path.join(base_dir, query_dir, f'TSS_output/all/budget_{budget_size}/target_{target_size}/conditional_{fxn}_{other_accent}/eta_{etaValue}/{similarity}/{feature_type}')
...
#        train_list = selected_list + query_list
        ...
        for item in selected_list:
            if accent_map[item['audio_filepath'].split('/')[-3]] == accent:
        ...
        list_total_selection.append(Counter([accent_map[item['audio_filepath'].split('/')[-3]] for item in selected_list]))
        
        with open(f'{run_dir}/train/train.json', 'w') as f:
            for line in train_list:
                f.write('{}\n'.format(json.dumps(line)))

#        plots(dirs, run_dir, ground_features, ground_features_Y, query_features, private_features, test_features, selected_indices, selected_gains, fxn)
...
def generate_greedyList(ground_kernel, query_kernel, query_query_kernel, private_kernel, private_private_kernel, query_private_kernel, fxn, budget_size, etaValue):
    print(f'\ncreating {fxn} object\n')
    if fxn == 'FLMI':
        obj1 = FacilityLocationConditionalMutualInformationFunction(n=len(ground_kernel), num_queries=query_kernel.shape[1], num_privates=private_kernel.shape[1], query_sijs=query_kernel, data_sijs=ground_kernel, private_sijs=private_kernel, magnificationEta=etaValue)
#    elif fxn == 'FL2MI':
...
#        obj1 = GraphCutMutualInformationFunction(n=len(ground_kernel), num_queries=query_kernel.shape[1], query_sijs=query_kernel)
        ...
        obj1 = LogDeterminantConditionalMutualInformationFunction(n=len(ground_kernel), num_queries=query_kernel.shape[1], num_privates=private_kernel.shape[1], lambdaVal=1, query_sijs=query_kernel, data_sijs=ground_kernel, query_query_sijs=query_query_kernel, private_sijs=private_kernel, private_private_sijs=private_private_kernel, query_private_sijs=query_private_kernel, magnificationEta=etaValue)
    else:
...
def preprocess(base_dir, target_size, budget_size, accent, other_accent, similarity, feature_type):
...
#    dirs = [f.name for f in os.scandir('/home/mayank/MTP/begin_again/Error-Driven-ASR-Personalization/CMU_expts/accent/') if f.is_dir()]
    ...
    private_dir = f'{other_accent}/manifests/'
    private_file_path = base_dir + private_dir + 'seed.json'
    private_list = [json.loads(line.strip()) for line in open(private_file_path)]
    private_features = load_features(private_file_path, feature_type)
    ...
    print(len(private_list), private_features.shape)
    ...
    query_query_kernel = create_kernel(query_features, metric=similarity, mode='dense')
    private_kernel = create_kernel(private_features, metric=similarity, mode='dense', X_rep=ground_features)
    private_private_kernel = create_kernel(private_features, metric=similarity, mode='dense')
    query_private_kernel = create_kernel(private_features, metric=similarity, mode='dense', X_rep=query_features)
    t2 = time.time()
    ...
    print('private_kernel: ', private_kernel.shape)
    print('private_private_kernel: ', private_private_kernel.shape)
    print('query_private_kernel: ', query_private_kernel.shape)

    return dirs, query_dir, ground_list, ground_features, ground_features_Y, ground_kernel, query_list, query_features, query_kernel, query_query_kernel, private_list, private_features, private_kernel, private_private_kernel, query_private_kernel, test_features 
...
parser.add_argument('--other_accent', type = str,  help ='private set')
...
fxn = args.fxn
accent = args.accent
other_accent = args.other_accent
feature_type = args.feature_type
base_dir = ''

dirs, query_dir, ground_list, ground_features, ground_features_Y, ground_kernel, query_list, query_features, query_kernel, query_query_kernel, private_list, private_features, private_kernel, private_private_kernel, query_private_kernel, test_features = preprocess(base_dir, target_size, budget_size, accent, other_accent, similarity, feature_type)
greedyList = generate_greedyList(ground_kernel, query_kernel, query_query_kernel, private_kernel, private_private_kernel, query_private_kernel, fxn, budget_size, etaValue)
compute_subset(dirs, base_dir, query_dir, ground_list, ground_features, ground_features_Y, query_list, query_features, private_list, private_features, test_features, greedyList, budget_size, target_size, accent, other_accent, fxn, similarity, etaValue, feature_type)
