...
#    expt_details=/all/budget_$b1/target_$tar/$fxn/$ft
   . scripts/error.sh $accent $b2 $file_dir $file_dir/train.json
   for run in 1 2 3
   do
       file_dir=$accent/all/budget_$b1/target_$tar/$fxn/eta_$eta/$sim/$ft/budget_$b2/error_model/run_$run
       mkdir -p $file_dir
       echo "---------------beginning finetuning----------------------------"
       cd "$finetunepath"
       . L2-scripts/finetune.sh $accent $file_dir
       echo "---------------beginning testing----------------------------"
       . L2-scripts/test.sh $accent $file_dir
       cd "$homepath"
   done
