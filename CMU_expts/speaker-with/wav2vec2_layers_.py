...
def extract_features_2(file_list, file_dir, layer_num):
    with open(file_dir.replace('.json', f'_w2v2_layer-{layer_num}_and_last.file'), 'ab') as f:
        for file in tqdm(file_list):
#            waveform, sample_rate = torchaudio.load(file['audio_filepath'])
            waveform, sample_rate = librosa.load(file['audio_filepath'], sr=16000)
            waveform = torch.tensor(waveform)
            speech_inp = tokenizer(waveform, return_tensors='pt', padding='longest').input_values.to(device)
            w2v2_features_layer_1 = model(speech_inp, output_hidden_states=True).hidden_states[layer_num - 1].mean(1)
            w2v2_features_layer_2 = model(speech_inp, output_hidden_states=True).hidden_states[-1].mean(1)
            w2v2_features = torch.cat((w2v2_features_layer_1, w2v2_features_layer_2), 1).cpu().detach().numpy()
            pickle.dump(w2v2_features, f)
...
for i in [1, 10]:
    ...
    #     print('seed_file_ending ...\n\n')
        ...
        extract_features_2(selection_list, selection_file_dir, layer_num)
