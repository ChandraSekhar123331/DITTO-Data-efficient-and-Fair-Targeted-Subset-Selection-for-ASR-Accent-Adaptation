...
def train(
            ...
            for data in eval_dataloader:
                ...
                if optim_level == 1:
                    with amp.disable_casts():
                        (
                            t_processed_signal_e,
                            t_processed_sig_length_e,
                        ) = audio_preprocessor(t_audio_signal_e, t_a_sig_length_e)
                else:
    ...
    audio_preprocessor = (
        model.module.audio_preprocessor
        if hasattr(model, "module")
        else model.audio_preprocessor
    )
    data_spectr_augmentation = (
        model.module.data_spectr_augmentation
        if hasattr(model, "module")
        else model.data_spectr_augmentation
    )
    jasper_encoder = (
        model.module.jasper_encoder
        if hasattr(model, "module")
        else model.jasper_encoder
    )
    ...
    while True:
        ...
        for data in train_dataloader:
            ...
            if batch_counter % args.gradient_accumulation_steps == 0:
                ...
                if step % args.train_frequency == 0:
                    ...
                    train_wer, train_wer_list = monitor_asr_train_progress(
                        e_tensors, labels=labels
                    )
                    print_once(
                        "Loss@Step: {0}  ::::::: {1}".format(step, str(average_loss))
                    )
                    print_once(
                        "Step time: {0} seconds".format(time.time() - last_iter_start)
                    )
                    other_inputs["summary_writer"].add_scalar(
                        "Loss/train", average_loss, step
                    )
        ...
        if args.num_steps is not None and step >= args.num_steps:
            break
        print_once(
            "Finished epoch {0} in {1}".format(epoch, time.time() - last_epoch_start)
        )
        epoch += 1
        ...
        if args.save_after_each_epoch:
            print_once(
                "Doing Evaluation after finishing epoch ....................... ......  ... .. . ."
            )
            e_wer, e_loss = eval()
...
def main(args):
    ...
    if multi_gpu:
        print_once(
            "DISTRIBUTED TRAINING with {} gpus".format(
    ...
    if args.batch_size % args.gradient_accumulation_steps != 0:
        raise ValueError(
            "gradient accumulation step {} is not divisible by batch size {}".format(
    ...
    other_inputs = (
        {}
    )  # other inputs to be sent to train() e.g. intial decoder parameters
    ...
    print_once(
        "Number of parameters in encoder: {0}".format(
            model.jasper_encoder.num_weights()
        )
    )
    print_once(
        "Number of parameters in decode: {0}".format(model.jasper_decoder.num_weights())
    )
    ...
    if args.lr_decay == "decay":
        fn_lr_policy = lambda s: lr_policy(
            args.lr, s, args.num_epochs * args.step_per_epoch
        )
    elif args.lr_decay == "warmup":
        fn_lr_policy = lambda s: warmup_decay_policy(
            args.lr, s, args.num_epochs * args.step_per_epoch
        )
    else:
        fn_lr_policy = None
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=200,
        type=int,
        help="number of iterations until doing evaluation on full dataset",
    )
    parser.add_argument(
        "--train_freq",
        dest="train_frequency",
        default=25,
        type=int,
    ...
    parser.add_argument(
        "--weight_decay", default=1e-3, type=float, help="weight decay rate"
    )
    parser.add_argument(
        "--train_manifest",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to directory containing wav files in manifest",
    )
    parser.add_argument(
        "--model_toml",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="minimum duration of audio samples for training and evaluation",
    )
    parser.add_argument(
        "--pad_to_max",
        action="store_true",
        default=False,
        help="pad sequence to max_duration",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["warmup", "decay", "none"],
        help="learning rate decay strategy",
    )
    parser.add_argument(
        "--cudnn", action="store_true", default=False, help="enable cudnn benchmark"
    )
    parser.add_argument(
        "--fp16",
        action="store_true",
    ...
    parser.add_argument(
        "--output_dir", type=str, required=True, help="saves results in this directory"
    )
    parser.add_argument(
        "--best_dir",
        type=str,
        required=True,
        help="saves the best ckpt in this directory",
    )
    parser.add_argument(
        "--ckpt", default=None, type=str, help="path to load a pre-trained ckpt"
    )
    parser.add_argument(
        "--resume_from_ckpt",
        action="store_true",
        default=False,
        help="resume training from ckpt's epoch",
    )
    parser.add_argument(
        "--load_optimizer_state",
        action="store_true",
        default=False,
        help="load optimizer's state from ckpt",
    )
    ...
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="max number of epochs to wait if performance does not improve",
    )
    parser.add_argument(
        "--turn_bn_eval",
        action="store_true",
        default=False,
        help="fix mean and variance of bn layers during training",
    )
    parser.add_argument(
        "--zero_infinity",
        action="store_true",
        default=False,
        help="turn on zero infinity in CTC loss",
    )
    parser.add_argument(
        "--save_after_each_epoch",
        action="store_true",
        default=False,
        help="save best ckpt only after each epoch",
    )
    parser.add_argument(
        "--freeze_first_n_blocks",
        type=int,
        default=0,
        help="freeze first n blocks in the jasper encoder",
    )
    parser.add_argument(
        "--use_cer", action="store_true", default=False, help="use cer for evaluation"
    )
