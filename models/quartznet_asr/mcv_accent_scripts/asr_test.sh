...
do 
	...
	do
		...
		echo 
		echo
		model_dir=$PRETRAINED_CKPTS/quartznet/finetuned/mcv-accent_"$accent"/all/budget_"$size"/target_"$target"/"$fxn"/eta_"$eta"/"$similarity"/"$feature"/run_"$run"/
		...
		output_dir=$DATA/$accent/manifests/TSS_output/all/budget_"$size"/target_"$target"/"$fxn"/eta_"$eta"/"$similarity"/"$feature"/run_"$run"/output/
