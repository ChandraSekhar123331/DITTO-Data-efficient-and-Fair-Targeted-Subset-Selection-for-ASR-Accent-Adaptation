...
do 
	...
	do
		...
		echo 
		echo
		model_dir=$PRETRAINED_CKPTS/quartznet/finetuned/$accent/within/budget_"$size"/target_"$target"/random/run_"$run"/
		...
		output_dir=$DATA/$accent/manifests/TSS_output/within/budget_"$budget"/target_"$target"/random/run_"$run"/output/
