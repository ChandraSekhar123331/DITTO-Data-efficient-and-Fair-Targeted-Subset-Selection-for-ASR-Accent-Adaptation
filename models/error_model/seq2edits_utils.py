# adapted from https://github.com/belambert/edit-distance/blob/master/edit_distance/code.py
...
def edit_distance(seq1, seq2, action_function=lowest_cost_action, test=operator.eq):
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            cost = (
                0
                if test(seq1[i - 1], seq2[j - 1])
                else (1 + abs(len(seq1[i - 1]) - len(seq2[j - 1])) / 1000)
            )
            # The costs
            ins_cost = v1[j - 1] + 1
            del_cost = v0[j] + 1
...
def ndiff(source, target, merge_insertions=True):
    ...
    for item in opcodes:
        if item[0] == "equal":
            ...
            item[0] == "replace"
        ):  # BREAK a substitution to delete (-) followed by insert (+)
            diff.append("- {}".format(source[src_id]))
            diff.append("+ {}".format(target[item[3]]))
        elif item[0] == "delete":
            diff.append("- {}".format(source[src_id]))
...
if __name__ == "__main__":
    x = "I like him , re also he like me ."
    y = "I like him . frwer gh also , he like ."
    print(edit_distance_backpointer(x.split(), y.split()))
    print()
