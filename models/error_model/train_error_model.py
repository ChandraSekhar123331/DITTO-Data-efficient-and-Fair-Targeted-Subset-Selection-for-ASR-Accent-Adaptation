...
from helpers import print_dict, save_model, warmup_decay_policy
from metrics import error_classifier_errors, get_precision_recall_f1, xent_loss
from model import ErrorClassifierPhoneBiLSTM_V2
from torch.utils.tensorboard import SummaryWriter

from data import error_classifier_collate_fn, generate_error_data_from_json
    ...
    with torch.no_grad():
        ...
        for data in eval_data_loader:
            ...
            (
                ...
                false_positives,
                false_negatives,
            ) = error_classifier_errors(predictions, error_positions, padding_positions)
...
def train(
    ...
    dev_loss, dev_error_rate, precision, recall, f1 = eval(
        dev_data_loader, model, args, device
    )
    best_f1 = f1
    ...
    for epoch in range(1, args.num_epochs + 1):
        for data in train_data_loader:
            ...
            if steps % args.train_frequency == 0:
                print(
                    "\t epoch: {} steps: {} train_precision: {:.2f} train_recall: {:.2f} train_f1: {:.2f} train_loss: {:.2f}\n".format(
                        epoch, steps, precision, recall, f1, train_loss
                    )
                )
                other_inputs["summary_writer"].add_scalar(
                    "Loss/train", train_loss.item(), steps
                )
                other_inputs["summary_writer"].add_scalar(
                    "Error/train", train_error_rate, steps
                )
                other_inputs["summary_writer"].add_scalar(
                    "precision/train", precision, steps
                )
        ...
        print(
            "epoch: {} steps: {} best_f1: {:.2f} dev_precision: {:.2f} dev_recall: {:.2f} dev_f1: {:.2f} dev_loss: {:.2f}\n".format(
                epoch, steps, best_f1, precision, recall, f1, dev_loss
            )
        )
    ...
    if not os.path.isdir(args.best_dir):
        ...
        copyfile(
            ...
            os.path.join(args.best_dir, file_name),
        )
...
def parse_args():
    ...
    #     type=str,
    #     required=True,
    #     help="path to output file containing references and hypothesis generated by ASR model",
    # )
    ...
    parser.add_argument(
        ...
        default=0.65,
        type=float,
        help="portion of data used for training error model, rest is used as dev",
    )
    parser.add_argument(
        "--pretrained_ckpt", default=None, type=str, help="path to pretrained ckpt"
    )
    parser.add_argument(
        "--input_size", default=64, type=int, help="size of phone embeddings"
    )
    parser.add_argument(
        "--hidden_size", default=64, type=int, help="size of hidden cells of lstm"
    )
    parser.add_argument(
        "--num_layers", default=4, type=int, help="number of lstm layers"
    )
    parser.add_argument(
        "--train_json_path",
        type=str,
        required=True,
        help="path to json on which we train the error model",
    )
    parser.add_argument(
        "--train_budget",
        type=int,
...
def main(args):
    ...
    print("loading data....")
    data = generate_error_data_from_json(
        args.train_json_path, use_pseudoTrans=args.pseudoTrans, skip_zero_CER=False
    )
    assert len(data) >= args.train_budget
    data = data[: args.train_budget]
    random.shuffle(data)
    ...
    phone_bilstm = ErrorClassifierPhoneBiLSTM_V2(
    ...
    optimizer = optim.Adam(
        phone_bilstm.parameters(), lr=args.lr, weight_decay=args.weight_decay
    )
    ...
    if args.lr_decay == "warmup":
        fn_lr_policy = lambda s: warmup_decay_policy(
            args.lr, s, args.num_epochs * steps_per_epoch
        )
    else:
        fn_lr_policy = None
