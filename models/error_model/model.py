...
from data import Phonemes, coarse_phone_to_fine_phone, phone_list
...
class ErrorClassifierPhoneBiLSTM_V2(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(
            input_size + VOWEL_EMBED_SIZE + FINE_EMBED_SIZE,
            hidden_size,
            num_layers,
            batch_first=True,
            dropout=0.1,
    ...
    def forward(self, inputs, input_lengths):
        # input: (B x T)
        ph_vowel_embeddings = self.vowel_embeddings(
            torch.tensor(self.vowel_mask, dtype=torch.int64, device=inputs.device)
        )  # (V x 8)
        ph_vowel_embeddings = torch.unsqueeze(ph_vowel_embeddings, 0)  # (1 x V x 8)

        ph_fine_embeddings = self.fine_embeddings(
            torch.tensor(self.fine_mask, dtype=torch.int64, device=inputs.device)
        )  # (V x 24)
