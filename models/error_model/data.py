...
class Phonemes(object):
    ...
    consonants = set.union(
        c_stops, c_afficates, c_fricatives, c_nasals, c_liquids, c_semivowels
    )
...
def get_phoneme_transcript(grapheme_transcript, markers=True):
    ...
    phoneme_transcript = [
        item if len(item) != 3 else item[:-1] for item in phoneme_transcript
    ]
...
def mask_phoneme_tokens(phoneme_sequence, masking_fraction):
    ...
    phoneme_sequence = phoneme_sequence[
        1:-1
    ]  # removing start and end, as they do not get masked
    ...
    masked_sequence = [
        MASK if i in mask_positions else item for i, item in enumerate(phoneme_sequence)
    ]
...
def collate_fn(batch, masking_fraction, padding_value=0):
    ...
    for i, phoneme_sequence in enumerate(batch):
        masked_sequence, masked_position = mask_phoneme_tokens(
            phoneme_sequence, masking_fraction
        )
    ...
    padding_positions = [
        [1] * len(item) + [0] * (max_length - len(item)) for item in phoneme_sequences
    ]
    masked_sequences = [
        item + [padding_value] * (max_length - len(item)) for item in masked_sequences
    ]
    phoneme_sequences = [
        item + [padding_value] * (max_length - len(item)) for item in phoneme_sequences
    ]
    masked_positions = [
        item + [padding_value] * (max_length - len(item)) for item in masked_positions
    ]

    return (
        torch.tensor(phoneme_sequences, dtype=torch.int64),
...
def error_classifier_collate_fn(batch, padding_value=0):
    ...
    error_sequences = [
        item + [padding_value] * (max_length - len(item)) for item in error_sequences
    ]
    ...
    for op in p_ops:
        ...
        if op == "C":
            ...
            (
                ...
                insert_left,
                insert_right,
            ) = __get_error_sequence_between_words(ref_word, hyp_word)
            if insert_left:
                error_sequence[-1] = 1
