...
accent = 'indian'
data_files = {
    "train": f"../mz-isca/expts/{accent}/manifests/w2v2_finetune/selection_train.csv", 
    "dev": f"../mz-isca/expts/{accent}/manifests/w2v2_finetune/dev.csv",
    "test": f"../mz-isca/expts/{accent}/manifests/w2v2_finetune/test.csv",
}
...
vocabs = mcv.map(extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=mcv.column_names["train"])
...
import json
with open(f'mcv/{accent}/pre/vocab.json', 'w') as vocab_file:
...
tokenizer = Wav2Vec2CTCTokenizer(f"mcv/{accent}/pre/vocab.json", unk_token="[UNK]", pad_token="[PAD]", word_delimiter_token="|")
tokenizer.save_pretrained(f'mcv/{accent}/pre/tokenizer')
...
#%%
#processor = Wav2Vec2Processor.from_pretrained(f"mcv/{accent}/pre/checkpoint-300")
model = Wav2Vec2ForCTC.from_pretrained(f"mcv/{accent}/pre/checkpoint-2600").cuda()
