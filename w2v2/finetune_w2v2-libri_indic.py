...
accent = 'hindi_rajasthani'
test1, test2 = accent.split('_')

data_files = {
    "train": f"../data/mix/{accent}/train_mix.csv", 
    "dev": f"../data/mix/{accent}/dev_mix.csv", 
#    "test": f"../data/mix/{accent}/test_kannada.csv",
}
data_files_test = {
    "test1": f"../data/mix/{accent}/test_{test1}.csv",
    "test2": f"../data/mix/{accent}/test_{test2}.csv",
}
mcv = load_dataset("csv", data_files=data_files, delimiter="\t", )
mcv_test = load_dataset("csv", data_files=data_files_test, delimiter="\t", )
...
mcv_test = mcv_test.map(convert_unicodes)
mcv_test = mcv_test.map(remove_special_characters)
mcv_test = mcv_test.map(upper_case)
...
mcv_test = mcv_test.map(add_audio_array)
...
mcv_test = mcv_test.map(prepare_dataset, remove_columns=mcv_test.column_names["test1"], num_proc=4)
...
training_args = TrainingArguments(
  output_dir=f'indic/{accent}/libri-100h',
...
model = Wav2Vec2ForCTC.from_pretrained(f"indic/{accent}/libri-100h/checkpoint-90").cuda()
...
results1 = mcv_test["test1"].map(map_to_result, remove_columns=mcv_test["test1"].column_names)
results2 = mcv_test["test2"].map(map_to_result, remove_columns=mcv_test["test2"].column_names)

print(f'{test1} results: ')
