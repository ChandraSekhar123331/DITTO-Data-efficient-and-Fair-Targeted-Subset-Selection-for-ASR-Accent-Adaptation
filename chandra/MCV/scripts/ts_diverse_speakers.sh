...
do
   echo
   echo
   echo "--------------------diverse selection from selection.json-----------------------"
   echo "$fxn"
   ...
   . scripts/diverse_speakers.sh $b2 data/$file_dir
   for run in 1 2 3
   do
       file_dir=$accent/all/budget_$b1/target_$tar/$fxn/$ft/budget_$b2/diverse_speakers/run_$run
       mkdir -p data/$file_dir
