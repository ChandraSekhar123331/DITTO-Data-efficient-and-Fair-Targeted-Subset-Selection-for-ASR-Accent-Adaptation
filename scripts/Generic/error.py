...
def finetune_error(config):
    ...
    PRETRAINED_CKPTS=os.path.join(config["HOME_PATH"], "models", "pretrained_checkpoints")
    for run in range(1, 4):
        SETTING_PATH=os.path.join(config["accent"], config["INFER_JSON_PATH"], "budget_{}".format(config["budget"]), "error_model", f"run_{run}")
        JSON_PATH = os.path.join(config["FULL_DATASET_PATH"], SETTING_PATH, "train.json")
        CKPT_PATH = os.path.join(PRETRAINED_CKPTS, "quartznet", "finetuned", config["dataset"], SETTING_PATH)
        VAL_JSON_PATH = os.path.join(config["FULL_DATASET_PATH"], config["accent"], "dev.json")
...
def test_error(config):
    ...
    for run in range(1, 4):
        ...
        JSON_PATH = os.path.join(config["FULL_DATASET_PATH"], SETTING_PATH, "train.json")
        CKPT_PATH = os.path.join(PRETRAINED_CKPTS, "quartznet", "finetuned", config["dataset"], SETTING_PATH)
        TEST_JSON_PATH = os.path.join(config["FULL_DATASET_PATH"], config["accent"], "test.json")
