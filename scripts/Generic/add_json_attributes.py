...
def add_json_attributes(config):
    ...
    OUT_PATH = os.path.join(config["FULL_DATASET_PATH"], SETTING_PATH, config["OUTPUT_JSON_NAME"])
    assert(config["INFER_JSON_NAME"][-5:] == ".json")
    JSON_BASE_NAME = config["INFER_JSON_NAME"][:-5]

    LOG_PATH = os.path.join(config["FULL_DATASET_PATH"], SETTING_PATH, "quartznet_outputs", f"{JSON_BASE_NAME}_out.txt")
    print(LOG_PATH)
