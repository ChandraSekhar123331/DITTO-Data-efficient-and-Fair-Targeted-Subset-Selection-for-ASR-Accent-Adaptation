...
def plot_TSNE(
    ...
    label_map = dict(
        enumerate(
            [
                x[0].upper() + x[1] + x[2] + ", " + x[:-8].split("_")[1][0].upper()
                for x in dirs
            ]
        )
    )
    ...
    plt.title(
        " ".join(
            ...
            + [f"ph-decay_{config_dict['phoneme_decay']}"]
        )
    )
    g.legend(
        loc="upper left",
...
def compute_subset(
    ...
    phoneme_feature_type = config_dict["phoneme_feature_type"]
    ...
    phoneme_decay = config_dict["phoneme_decay"]
    ...
    output_dir = os.path.join(
        ...
        "all",
        f"dim_phoneme_gains",
        f"tau_{phoneme_decay}",
        "-".join(
            [
                f"{accent}={round(fraction, 3)}"
                for accent, fraction in zip(
                    config_dict["query_set"], config_dict["query_set_percent"]
                )
            ]
        ),
        ...
        f"phoneme_{phoneme_feature_type}",
...
# %%
def get_phoneme_score(indices, phoneme_features: np.ndarray, tau):
    if len(indices) == 0:
        return 0

    selected_total_freqs= np.sum(phoneme_features[indices, :], axis = 0).reshape(1, -1)
    return np.sum(1 - np.exp(-selected_total_freqs*tau))

# %%
def get_score(indices, obj: FacilityLocationMutualInformationFunction, phoneme_features: np.ndarray, tau):
    # submod_time, phoneme_time = 0, 0
    # t0 = time.time()
    submod_score = obj.evaluate(set(indices))
    # t1 = time.time()
    phoneme_score = get_phoneme_score(indices, phoneme_features, tau=tau)
    # t2 = time.time()
    # print(submod_score, phoneme_score)
    # submod_time = t1 - t0
    # phoneme_time = t2 - t1
    return submod_score * phoneme_score, submod_score, phoneme_score
    

#%%
def maximize(budget, submod_obj: FacilityLocationVariantMutualInformationFunction, ground_phoneme_features, ground_Y, dirs, tau):
    num_samples = submod_obj.n
    if(budget > num_samples):
        raise Exception(f"budget = {budget} should be less than num_samples = {num_samples}")
    assert(len(ground_phoneme_features) == num_samples)
    selected_indices = []
    remaining_indices = set(range(num_samples))
    greedy_gains = []
    for _ in tqdm(range(budget)):
        # t0 = time.time()
        # total_sub, total_phon = 0, 0
        initial_score, oldsub, oldphon = get_score(selected_indices, submod_obj, ground_phoneme_features, tau=tau) #submod_obj.evaluate(selected_indices) * phoneme_score(selected_indices, ground_phoneme_features)
        # total_sub += tsub
        # total_phon += tphon
        best_new_score = -1e18
        best_new_ind = -1
        for rem_ind in tqdm(remaining_indices):
            selected_indices.append(rem_ind)
            new_score, newsub, newphon = get_score(selected_indices, submod_obj, ground_phoneme_features, tau=tau) #submod_obj.evaluate(selected_indices) * phoneme_score(selected_indices, ground_phoneme_features)
            # print(dirs[ground_Y[rem_ind]], new_score - initial_score, newsub - oldsub, newphon - oldphon)
            # total_sub += tsub
            # total_phon += tphon
            selected_indices.pop()
            if new_score > best_new_score:
                best_new_score = new_score
                best_new_ind = rem_ind
        # t1 = time.time()
        # total = t1 - t0
        # print(total, total_sub, total_phon, total - total_sub-total_phon, flush=True)
        
        # print("best: ", f"Accent = {dirs[ground_Y[best_new_ind]]}", f"Score gain = {best_new_score - initial_score}")
        assert(best_new_ind >= 0)
        selected_indices.append(best_new_ind)
        remaining_indices.remove(best_new_ind)
        greedy_gains.append(best_new_score - initial_score)

    return tuple(zip(selected_indices, greedy_gains))
...
def generate_greedyList(
    ...
    greedyList = maximize(
        ...
        # show_progress=True,
        submod_obj=obj1,
        ground_phoneme_features = config_dict["ground_phoneme_features"],
        ground_Y=config_dict["ground_list_Y"],
        dirs=config_dict["dirs"],
        tau=config_dict["phoneme_decay"],
    )
...
def preprocess(config_dict):
    ...
    for query_accent in config_dict["query_set"]:
        ...
        total_query_phoneme_features.append(
            load_features(
                accent=query_accent,
                feature_type=config_dict["phoneme_feature_type"],
                filename="seed",
            )
        )
    ...
    for ind, query_accent in enumerate(config_dict["query_set"]):
        ...
        query_phoneme_features.extend(
            total_query_phoneme_features[ind][
                : int(
                    config_dict["target_size"] * config_dict["query_set_percent"][ind]
                )
            ]
    ...
    query_phoneme_features = [_.reshape(1, -1) for _ in query_phoneme_features]
    ...
    query_phoneme_features = np.concatenate(query_phoneme_features, axis=0)
    ...
    query_phoneme_features = query_phoneme_features[:target_size]
    # print(type(query_accent_features), type(query_content_features))
    ...
    for i, _dir in enumerate(dirs):
        ...
        ground_phoneme_features.append(
            load_features(
                accent=_dir,
                feature_type=config_dict["phoneme_feature_type"],
                filename="selection",
    ...
    ground_phoneme_features = np.concatenate(ground_phoneme_features, axis=0)
    ...
    return {
        ...
        "query_phoneme_features": query_phoneme_features,
        "ground_phoneme_features": ground_phoneme_features,
    }
...
def do_expt(**kwargs):
    ...
    config_dict["phoneme_feature_type"] = kwargs["phoneme_feature_type"]
    config_dict["use_phoneme_gains"] = kwargs["use_phoneme_gains"]
    config_dict["phoneme_decay"] = kwargs["phoneme_decay"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fxn",
        type=str,
        help="submodular function",
        required=True,
        choices=["FL1MI", "FL2MI", "GCMI", "LogDMI"],
    )
    parser.add_argument(
        "--target", type=int, help="target", required=True, choices=[20, 30]
    )
    parser.add_argument(
        "--phoneme_decay", type=float, help="phoneme_decay_param", required=True,
    )
    args = parser.parse_args()
    args = vars(args)
    fxn_list = [args["fxn"]]
    target = args["target"]
    phoneme_decay = args["phoneme_decay"]
    # parser.add_argument(
    ...
    dirs = [
        # (["gujarati_female_english"], [1]),
        # (["assamese_female_english"], [1]),
        # (["hindi_male_english"], [1]),
        # (["kannada_male_english"], [1]),
        # (["malayalam_male_english"], [1]),
        # (["manipuri_female_english"], [1]),
        (["rajasthani_male_english"], [1]),
        (["tamil_male_english"], [1]),
        ...
        # (
        #     ["assamese_female_english", "kannada_male_english", "tamil_male_english"],
        #     [2 / 10, 3 / 10, 5 / 10],
        # ),
    ]
        ...
        for accent in query_set:
            ...
            for fxn in fxn_list:
                ...
                param_dict = {
                    ...
                    "fxn": fxn,
                    "use_phoneme_gains": True,
                    "phoneme_feature_type": "pseudo-phfreq-1gram",
                    "accent": accent,
