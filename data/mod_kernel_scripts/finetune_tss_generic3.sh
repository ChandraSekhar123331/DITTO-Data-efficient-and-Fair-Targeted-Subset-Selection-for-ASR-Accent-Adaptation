...
declare -a accents=('rajasthani_male_english' 'tamil_male_english' 'gujarati_female_english' 'kannada_male_english' 'malayalam_male_english' 'manipuri_female_english')
# declare -a functions=('GCMI' 'FL2MI' 'FL1MI' 'LogDMI')
declare -a functions=('GCMI')
...
do
    ...
    do
        ...
        do 
            ...
            file_dir=$accent/all/budget_$budget/target_$target/"$fxn"_etaScale_"$eta_scale"/accent_"$accent_feature_type"/content_"$content_feature_type"/kernel_g=accent_gq=accent_qq=accent/accent_"$accent_similarity"/content_"$content_similarity"/run_"$run"/
            mkdir -p $file_dir
            cp $accent/all/budget_$budget/target_$target/"$fxn"_etaScale_"$eta_scale"/accent_"$accent_feature_type"/content_"$content_feature_type"/kernel_g=accent_gq=accent_qq=accent/accent_"$accent_similarity"/content_"$content_similarity"/train.json $file_dir
            cd "$finetunepath"
